...
(`arp_table`, `snmp_inventory` with 802.1Q VLAN, `lldp_cdp`), NetBIOS/mDNS
(`name_lookup`), DNS (`dns_lookup`), and DHCP lease files (`dhcp_leases`) provide
unprivileged passive imports, and a `combined` scan fuses them per host.

Further passive integrations (firewall and controller imports) remain planned
optional integrations. Light IPAM never requires connection to a DHCP server —
`dhcp_leases` is opt-in and reads a mounted lease file rather than talking to the
server.
